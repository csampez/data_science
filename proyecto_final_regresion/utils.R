...
ggally_cor <- function(data, mapping, corAlignPercent = 0.6, ...){
  ...
  if(colorCol != "ggally_NO_EXIST" && colorCol %in% colnames(data)) {
    ...
    cor_signif <- symnum(cor_obj$p.value, corr = FALSE,
               ...
               symbols = c("***", "**", "*", ".", " "))
    cor_text_size <- 5 + 2.5*abs(cor_obj$estimate)
...
get_data <- function(path){
  #print(path)
  aux<-read.csv(path,stringsAsFactor=FALSE,as.is=TRUE,na.strings=c("NA","NaN", " ","ND"))
  #aux<-read_csv(path)
  #aux<-sapply(aux,})
  aux
  }

remove_tab<-function(col){
  gsub('\t','',col)
}

get_clean<-function(path,colclass){
  aux<-sapply(read.csv(path, colClasses=colclass),remove_tab)
  as.data.frame(aux)
}
